    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3.5" id="dealer-form">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <select {...register("yearsInBusiness")}
            className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-brand-green bg-white text-gray-500">
      ...
      <div>
        <select {...register("interestedProducts")}
          className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-brand-green bg-white text-gray-500">
          <option value="">Interested in Products *</option>
          {PRODUCT_OPTIONS.map((opt) => (
            <option key={opt} value={opt}>{opt}</option>
          ))}
        </select>
        {errors.interestedProducts && <p className="text-red-500 text-xs mt-1">{errors.interestedProducts.message}</p>}
      </div>

      {/* Business type */}
      <div>
        <p className="text-xs font-heading font-semibold text-gray-500 mb-2">Type of Business *</p>
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <label key={type.value} className="flex items-center gap-2 cursor-pointer text-sm text-brand-text">
              <input type="radio" value={type.value} {...register("businessType")} className="accent-brand-green" />
              {type.label}
            </label>
          ))}
        </div>
      ...
      <textarea {...register("message")} placeholder="Message / Additional Information (Optional)"
        rows={3}
        className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:border-brand-green resize-none" />
